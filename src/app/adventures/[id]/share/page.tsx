...
export default function ShareAdventurePage({ params }: SharePageProps) {
  ...
  return (
    <main className="min-h-screen bg-slate-50 px-6 py-10"><div className="mx-auto max-w-2xl"><Link className="text-sm font-semibold text-emerald-700" href="/dashboard">← Dashboard</Link><h1 className="mt-8 text-4xl font-semibold tracking-tight text-slate-950">Share your adventure</h1><p className="mt-3 text-slate-600">Send each contact their private status link. The links expire after 30 days.</p>{pending && <p className="mt-8 text-slate-600">Preparing links...</p>}{error && <p className="mt-8 text-red-700">{error}</p>}<div className="mt-8 space-y-4">{links.map((link) => <div className="rounded-2xl border border-slate-200 bg-white p-5" key={link.url}><p className="font-semibold text-slate-950">{link.name}</p><p className="mt-1 text-xs text-slate-500">{link.phone ?? link.email ?? "No notification destination configured"}</p><input className="mt-4 w-full rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-xs text-slate-600" readOnly value={link.url} /></div>)}</div>{notifications.length > 0 && <section className="mt-8 rounded-2xl border border-slate-200 bg-white p-5"><h2 className="font-semibold text-slate-950">Notification delivery</h2><div className="mt-3 space-y-2">{notifications.map((notification) => <div className="flex items-center justify-between gap-4 text-sm" key={notification.id}><span className="text-slate-600">{notification.channel} · {notification.recipient}</span><span className={notification.status === "FAILED" ? "font-semibold text-red-700" : "font-semibold text-emerald-700"}>{notification.status.toLowerCase()}{notification.errorMessage ? `: ${notification.errorMessage}` : ""}</span></div>)}</div></section>}{id && <p className="mt-8 text-sm text-slate-500">Delivery status is recorded for follow-up. Configure SMS/email providers before relying on automated delivery.</p>}</div></main>
  );
}
