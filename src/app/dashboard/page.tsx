...
export default async function DashboardPage() {
  ...
  const [contacts, adventures] = await Promise.all([
    ...
    prisma.adventure.findMany({ where: { userId: user.id }, select: { id: true, timezone: true, startAt: true, expectedReturnAt: true, status: true, hike: { select: { id: true, title: true } }, contacts: { select: { contact: { select: { id: true, name: true } } } }, events: { select: { id: true, type: true, createdAt: true }, orderBy: { createdAt: "asc" } }, comments: { select: { id: true, body: true, createdAt: true, authorUser: { select: { name: true } }, authorContact: { select: { name: true } } }, orderBy: { createdAt: "asc" } } }, orderBy: { expectedReturnAt: "asc" } }),
  ]);
  ...
  return (
    <main className="min-h-screen bg-slate-50 px-6 py-10">
      <div className="mx-auto max-w-5xl">
        ...
        <AdventuresPanel hikes={hikes.map((hike) => ({ id: hike.id, title: hike.title }))} contacts={contacts} initialAdventures={adventures.map((adventure) => ({ ...adventure, startAt: adventure.startAt?.toISOString() ?? null, expectedReturnAt: adventure.expectedReturnAt.toISOString(), events: adventure.events.map((event) => ({ ...event, createdAt: event.createdAt.toISOString() })), comments: adventure.comments.map((comment) => ({ ...comment, createdAt: comment.createdAt.toISOString() })) }))} timezone={user.timezone} />
      </div>
    </main>
  );
}
