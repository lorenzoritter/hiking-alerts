...
export function AdventuresPanel({ hikes, contacts, initialAdventures, timezone }: AdventuresPanelProps) {
  ...
  const initialStart = dateTimeParts(1, timezone);
  ...
  const [dateModal, setDateModal] = useState<"start" | "return" | null>(null);
  ...
  async function createAdventure(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (!hikeId || !startDateIso || !returnDateIso || !isValidTime(startTime) || !isValidTime(returnTime)) {
      setError(!hikeId ? "Choose a hike before creating the adventure." : "Use dates like 15 Aug 2026 and times like 18:30.");
      setPending(false);
      return;
    }
    try {
  ...
  return (
    <section className="mt-8 rounded-2xl border border-slate-200 bg-white p-7 shadow-sm">
      ...
      {hikes.length === 0 || contacts.length === 0 ? <p className="mt-4 rounded-xl bg-amber-50 px-4 py-3 text-sm text-amber-900">Create a hike and at least one emergency contact before sharing an adventure.</p> : <form className="mt-6 space-y-4" onSubmit={createAdventure}>
        <div className="relative text-sm font-medium text-slate-700"><span>Hike</span><button aria-controls="hike-options" aria-expanded={hikeOpen} aria-haspopup="listbox" className="mt-2 flex w-full items-center justify-between rounded-xl border border-slate-300 bg-white px-4 py-3 text-left font-normal text-slate-950" onClick={() => { setActiveHikeIndex(Math.max(0, hikes.findIndex((hike) => hike.id === hikeId))); setHikeOpen((open) => !open); }} onKeyDown={(event) => { if (event.key === "ArrowDown") { event.preventDefault(); setHikeOpen(true); hikeOptions.current[activeHikeIndex]?.focus(); } else if (event.key === "Escape") setHikeOpen(false); }} type="button">{hikes.find((hike) => hike.id === hikeId)?.title ?? "Choose a hike"}<span aria-hidden="true">⌄</span></button>{hikeOpen && <div className="absolute left-0 top-full z-20 mt-1 max-h-60 w-full overflow-auto rounded-xl border border-slate-300 bg-white p-1 text-slate-950 shadow-lg" id="hike-options" role="listbox" aria-label="Choose a hike">{hikes.map((hike, index) => <button aria-selected={hike.id === hikeId} className={`block w-full rounded-lg px-3 py-2 text-left ${hike.id === hikeId ? "bg-emerald-50 font-semibold text-emerald-800" : "hover:bg-slate-50"}`} key={hike.id} onClick={() => { setHikeId(hike.id); setActiveHikeIndex(index); setHikeOpen(false); }} onKeyDown={(event) => { if (event.key === "ArrowDown" || event.key === "ArrowUp") { event.preventDefault(); const next = (index + (event.key === "ArrowDown" ? 1 : -1) + hikes.length) % hikes.length; setActiveHikeIndex(next); hikeOptions.current[next]?.focus(); } else if (event.key === "Escape") setHikeOpen(false); }} ref={(element) => { hikeOptions.current[index] = element; }} role="option" type="button">{hike.title}</button>)}</div>}</div>
        <div className="grid gap-4 sm:grid-cols-2"><fieldset className="rounded-xl border border-slate-200 p-3"><legend className="px-1 text-sm font-medium text-slate-700">Departure</legend><div className="grid grid-cols-2 gap-2"><input aria-label="Departure date" aria-haspopup="dialog" className="min-w-0 rounded-lg border border-slate-300 px-3 py-2 font-normal text-slate-950" onClick={() => setDateModal("start")} placeholder="15 Aug 2026" readOnly type="text" value={startDate} required /><input aria-label="Departure time (24-hour)" className="min-w-0 rounded-lg border border-slate-300 px-3 py-2 font-normal text-slate-950" inputMode="numeric" maxLength={5} placeholder="18:30" type="text" value={startTime} onChange={(event) => updateTime(setStartTime, event.target.value)} required /></div></fieldset><fieldset className="rounded-xl border border-slate-200 p-3"><legend className="px-1 text-sm font-medium text-slate-700">Expected return</legend><div className="grid grid-cols-2 gap-2"><input aria-label="Expected return date" aria-haspopup="dialog" className="min-w-0 rounded-lg border border-slate-300 px-3 py-2 font-normal text-slate-950" onClick={() => setDateModal("return")} placeholder="15 Aug 2026" readOnly type="text" value={returnDate} required /><input aria-label="Expected return time (24-hour)" className="min-w-0 rounded-lg border border-slate-300 px-3 py-2 font-normal text-slate-950" inputMode="numeric" maxLength={5} placeholder="18:30" type="text" value={returnTime} onChange={(event) => updateTime(setReturnTime, event.target.value)} required /></div></fieldset></div>
        ...
        <fieldset><legend className="text-sm font-medium text-slate-700">Emergency contacts</legend><div className="mt-2 grid gap-2 sm:grid-cols-2">{contacts.map((contact) => <label className="flex items-center gap-2 text-sm text-slate-700" key={contact.id}><input checked={selectedContacts.includes(contact.id)} onChange={(event) => setSelectedContacts((current) => event.target.checked ? [...current, contact.id] : current.filter((id) => id !== contact.id))} type="checkbox" />{contact.name}</label>)}</div></fieldset>
         ...
         {dateModal && <div aria-labelledby="date-modal-title" aria-modal="true" className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/40 p-4" role="dialog"><div className="w-full max-w-sm rounded-2xl bg-white p-6 shadow-xl"><div className="flex items-center justify-between"><h2 className="text-lg font-semibold text-slate-950" id="date-modal-title">Choose {dateModal === "start" ? "departure" : "return"} date</h2><button aria-label="Close date picker" className="text-2xl leading-none text-slate-500" onClick={() => setDateModal(null)} type="button">×</button></div><input autoFocus aria-label="Date" className="mt-5 block w-full rounded-xl border border-slate-300 px-4 py-3 text-slate-950" type="date" value={dateModal === "start" ? parseDateInput(startDate) ?? "" : parseDateInput(returnDate) ?? ""} onChange={(event) => { const display = formatDateInput(event.target.value); if (dateModal === "start") setStartDate(display); else setReturnDate(display); setDateModal(null); }} /></div></div>}
      </form>}
